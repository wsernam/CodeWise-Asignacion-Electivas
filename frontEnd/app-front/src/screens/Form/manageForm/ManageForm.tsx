...
const ManageForm: React.FC = () => {
    ...
    const hasSelectedElectives = Object.values(selected).some(arr => arr.length > 0);

    // Maneja el cambio de fechas
    const handleDateChange = (values: any) => setDates(values);

    // Maneja el cambio de selección de electivas
    const handleCheckboxChange = (program: string, codigo: string, checked: boolean) => {
        setSelected(prev => {
    ...
    const handleToggle = () => {
        if (enabled) {
            ...
            if (!hasSelectedElectives) {
                setWarning({ open: true, message: "Debe seleccionar al menos una electiva antes de habilitar el formulario." });
                return;
            }
        ...
        if (confirmAction === 'disable') {
            ...
            if (activeForm) {
                try {
                    ...
                    setWarning({ open: true, message: "Error al deshabilitar el formulario." });
                }
            }
            ...
            try {
                ...
                setWarning({ open: true, message: "Error al habilitar el formulario." });
            }
        }
    ...
    const handleSave = async () => {
        if (!hasSelectedElectives) {
            setWarning({ open: true, message: "Debe seleccionar al menos una electiva antes de guardar la configuración." });
            return;
        }
        ...
        try {
            ...
            setWarning({ open: true, message: "Configuración del formulario guardada." });
        } catch (error) {
        ...
        <div className='auth-page'>
            ...
            <div className='auth-page-content'>
                <Card padding="xl" className="electives-card">
                    ...
                    <h2 className="electives-title">Gestión del Formulario de Asignación de Electivas</h2>
                        ...
                        {Object.keys(electivesByProgram).map(program => (
                            <div className="manage-form-program-card" key={program}>
                                <div className="manage-form-program-title">
                                    <b>{program}</b>
                                    <span style={{ color: "#1f297f", marginLeft: 6 }}>Electivas:</span>
                                </div>
                                <div className="manage-form-electives-list">
                                    {electivesByProgram[program].map((e: any) => (
                                        <label className="manage-form-checkbox-label" key={e.codigo}>
                                            <input
                                                type="checkbox"
                                                checked={selected[program]?.includes(e.codigo) || false}
                                                onChange={ev => handleCheckboxChange(program, e.codigo, ev.target.checked)}
                                            />
                                            {e.nombre} <span style={{ color: "#888" }}>({e.codigo})</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
